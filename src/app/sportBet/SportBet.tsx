...
import sportBet from '@/data/sportBet.json';
import Link from 'next/link';

export const SportBet = () => {
  const [selectedBets, setSelectedBets] = useState<{[key: string]: {selection: string, odds: number}}>({});
  ...
  const toggleBetSelection = (matchId: string, selection: string, odds: number) => {
    setSelectedBets(prev => {
      const newSelections = {...prev};
      
      if (newSelections[matchId] && newSelections[matchId].selection === selection) {
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="lg:w-3/4">
            <div className="bg-white rounded-lg shadow-md p-6 mb-6">
              ...
              <p className="text-gray-600 mb-4">Find the best odds across multiple sports</p>
              
              {/* Sports navigation */}
              <div className="flex overflow-x-auto pb-2 mb-6">
                <div className="flex space-x-2">
                  {Array.from(new Set(sportBet.map(sport => sport.category))).map((category) => (
                    <button 
                      key={category}
              ...
              {/* Featured matches */}
              <div className="mb-8">
                <h2 className="text-xl font-semibold border-b border-gray-200 pb-2 mb-4">Featured Matches</h2>
                
                {sportBet.filter(sport => sport.featured).map((sport) => (
                          ...
                          <div className="flex flex-col sm:flex-row items-center justify-between">
                            ...
                            <div className="flex space-x-2 w-full sm:w-auto justify-between">
                              <button
                                ...
                                onClick={() => toggleBetSelection(match.id, match.team1, match.odds.team1)}
                              >
                                <div className="text-sm font-medium">{match.team1}</div>
                                <div className="text-lg font-bold">{match.odds.team1.toFixed(2)}</div>
                              </button>
                              ...
                              {match.odds !== undefined && (
                                <button
                                  ...
                                  onClick={() => toggleBetSelection(match.id, 'Draw', match.odds.team1)}
                                >
                              ...
                              <button
                                ...
                                onClick={() => toggleBetSelection(match.id, match.team2, match.odds.team2)}
                              >
                                <div className="text-sm font-medium">{match.team2}</div>
                                <div className="text-lg font-bold">{match.odds.team2.toFixed(2)}</div>
                              </button>
                            </div>
                          </div>
                          
                          <div className="mt-2 text-right">
                            <Link href={`/match/${match.id}`} className="text-blue-600 text-sm hover:underline">
              ...
              {sportBet.filter(sport => !sport.featured).map((sport) => (
                <div key={sport.id} className="mb-6">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <Link href={`/sports/${sport.slug}`} className="text-blue-600 text-sm hover:underline">
                      View all
                    </Link>
                  </div>
            ...
            <div className="bg-white rounded-lg shadow-md sticky top-6">
              ...
              {Object.keys(selectedBets).length > 0 ? (
                <div className="p-4">
                  <div className="space-y-3 mb-4">
                    {Object.entries(selectedBets).map(([matchId, bet]) => {
                      const match = sportBet.flatMap(sport => sport.matches).find(m => m.id === matchId);
                      if (!match) return null;
                      
                      return (
                        <div key={matchId} className="bg-gray-50 p-3 rounded relative">
                          <button 
                            className="absolute top-2 right-2 text-gray-500 hover:text-red-500"
                            onClick={() => toggleBetSelection(matchId, bet.selection, bet.odds)}
                          >
                          ...
                          <div className="flex justify-between mt-2">
                            <span className="text-sm">{bet.selection} (1X2)</span>
                  ...
                  <div className="bg-gray-100 p-3 rounded mb-4">
                    <div className="flex justify-between mb-2">
                      ...
                      <span className="font-medium">Accumulator ({Object.keys(selectedBets).length})</span>
                    </div>
                    <div className="flex justify-between mb-2">
                ...
                <div className="p-4 text-center">
                  ...
                  <p className="text-sm text-gray-400 mt-1">Click on any odds to add a selection</p>
                </div>
              )}
            </div>
          </div>
